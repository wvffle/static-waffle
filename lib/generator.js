  ...
  .action((dir, cmd) => {
    ...
    for (const componentName of fs.readdirSync(componentsDir)) {
      ...
      if (fs.existsSync(cssFile)) {
        styledComponents[componentName] = true
        ...
        renderer.render((err, css) => {
          if (err) {
            signale.error(`${templateFile} -> ${dir}/components/${componentName}.css`)
            return signale.error(err)
          }

          fs.outputFileSync(`${dir}/components/${componentName}.css`, `${css}`)
          signale.success(`${templateFile} -> ${dir}/components/${componentName}.css`)
        })
      }
    }
